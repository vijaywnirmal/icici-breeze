...
def create_strategy(payload: StrategyPayload) -> Dict[str, Any]:
	try:
		...
		_ = Strategy.from_dict(payload.json)
		with get_conn() as conn:
			...
			row = conn.execute(
				...
				{"name": payload.name, "description": payload.description, "json": json.dumps(payload.json)},
			)
		...
		for fname in ("ma_crossover.json", "rsi_ob_os.json", "breakout.json"):
			...
			with open(path, 'r', encoding='utf-8') as fh:
				try:
					...
					continue
		return success_response("Templates", items=items)
	except Exception as exc:
